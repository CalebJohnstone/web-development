<?php
    ...
    class RedPillAPI{
        ...
        public function handleRequest(){
            //security: first strip any tags from $_POST

            //array_walk_recursive => will recursively remove all tags --> to account for nested arrays
            array_walk_recursive($_POST, $this->stripTags);

            if(isset($_POST["key"])){
                if(!isset($_POST["type"])){
                    $this->errorStatus = "Error: did not provide the type parameter";
                ...
                if($_POST["type"] === "info" || $_POST["type"] === "login"){
                    //check if the API_key is valid --> mainKey used to make sure it is not a malicious request --> for type={info, login}
                    $db = Database::instance();
                    $mainKey = $db->getMainKey();

                    if(strcmp($_POST["key"], $mainKey) !== 0){
                        $this->errorStatus = "Error: invalid key for the " . $_POST["type"] . " type";
                        return $this->response(""); 
                    }
                }
                ...
                $this->errorStatus = "Error: did not use the key parameter";
                return $this->response("");
            }
        }   
